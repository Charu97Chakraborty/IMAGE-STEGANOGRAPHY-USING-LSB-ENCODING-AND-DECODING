...
Status read_and_validate_decode_args(char *argv[], DecodeInfo *decInfo)
{
    ...
    {
        printf("Please mention steged image file correctly Eg:stego.bmp\n");
        // returns failure
        return e_failure;
    }
...
Status do_decoding(DecodeInfo *decInfo)
{
    ...
    {
        printf("Decoding file extn size successful\n");
    }
    else
    {
        printf("Decoding file extn size failed\n");
        return e_failure;
    }
    if (decode_file_extn(decInfo->size_image_data, decInfo) == e_success)
    {
        printf("Decoding file extn successful\n");
        if (decInfo->output_fname == NULL)
        {
            ...
            printf("Output file name not mentioned / unsupported. Creating %s as default with decoded file extension\n", decInfo->output_fname);
            // opening output file
            decInfo->fptr_output = fopen(decInfo->output_fname, "w");
            printf("Opened %s\n", decInfo->output_fname);
        }
    ...
    {
        printf("Decoding file extn failed\n");
        return e_failure;
    }
    if (decode_file_size(decInfo) == e_success)
    {
        printf("Decoding file size successful\n");
    }
    else
    {
    ...
    {
        printf("Decoding file data successful\n");
    }
    else
    {
        printf("Decoding file data failed\n");
        return e_failure;
    }
    return e_success;
}